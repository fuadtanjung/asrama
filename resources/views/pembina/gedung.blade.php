...
@section('content')
    <div class="container mt-1">
        <div class="card" style="width: 70%">
            <div class="card-header">Gedung</div>
            <br>
            <div class="container-sm">
                <button type="button" class="btn btn-primary btn-sm legitRipple" data-toggle="modal" data-target="#input_gedung">
            ...
            <div class="modal fade" id="input_gedung" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel">Gedung</h5>
                            <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                        </div>
                        <div class="modal-body">
                            <form id="form_gedung" method="post">
                                @csrf
                                <div class="form-group">
                                    <label for="exampleFormControlInput1">
                                        Nama Gedung
                                    </label>
                                    <input class="form-control form-control-solid" id="nama_gedung" name="nama_gedung" type="text">
                                </div>
                                <div class="form-group">
                                    <label for="exampleFormControlInput1">
                                        Nama Fakultas
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-primary" type="button" id="submit_gedung" aksi="input">Submit
                            </button>
                        </div>
                    </div>
                </div>
...
@section('script')
    <script type="text/javascript">
        ...
        function resetFormGedung() {
            $("#form_gedung")[0].reset();
        }

        $(window).on('load', function () {
            loadData();
            $('#submit_gedung').click(function () {
                var aksi = $("#submit_gedung").attr("aksi");
                if(aksi=="input"){
                    $.ajax({
                        url: "{{ url('/gedung/input') }}",
                        type: "post",
                        data: new FormData($('#form_gedung')[0]),
                        async: false,
                        cache: false,
                        contentType: false,
                        processData: false,
                    ...
                    var id_gedung= $("#submit_gedung").attr("idgedung");
                    $.ajax({
                        url: "{{ url('/gedung/edit') }}/"+id_gedung,
                        type: "post",
                        data: new FormData($('#form_gedung')[0]),
                        async: false,
                        cache: false,
                        ...
                        success: function (response) {
                            ...
                            if(pesan.error != null){
                                ...
                                $('#submit_gedung').attr("data-aksi","input");
                            }
                        },
                        fail: function () {
                            iziToast.error({
            ...
            $('#input_gedung').on('hidden.bs.modal', function () {
                resetFormGedung();
                $("#submit_gedung").attr("aksi","input");
                $('#submit_gedung').removeAttr("idgedung");
            });
        })
    </script>
@endsection
